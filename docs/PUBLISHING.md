...
`main` holds the editable source, tests, examples, architecture notes and the reproducible standalone HTML. GitHub Pages currently deploys the generated artifact through GitHub Actions. `gh-pages` contains an initial static-site snapshot and is available for branch-based publishing; it is not the active source while Pages uses Actions. There are no runtime package or CDN dependencies.
...
3. Read the repository's existing Pages configuration. Actions-based Pages deploys the artifact with the official deploy-pages action and a short-lived OIDC token. The alternative branch-based configuration updates `gh-pages` without force-pushing and explicitly requests a Pages build.
...
No personal access token, external hosting service or long-lived deployment secret is required. The build uses read-only repository access; the Actions-publishing job has pages/id-token write permission, while the alternative branch-publishing job has only contents/pages write permission. Deployments are serialized.

The verified configuration uses Settings > Pages > Source > GitHub Actions. The workflow also supports selecting `gh-pages` at `/` as a branch-based source. It never changes repository administration settings itself.
...
Use the workflow logs and final verification job rather than assuming a branch push means the site is live. In the alternative branch-based configuration, Pages builds are requested explicitly because pushes made with the workflow's GitHub token do not themselves trigger another Pages workflow. Keep `gh-pages` free of hand-edited application files; edit `src/`, run `npm run build`, and commit both source and generated standalone HTML to `main`.
